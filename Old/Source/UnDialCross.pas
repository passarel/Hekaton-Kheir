...
uses
  Classes, ComCtrls, Controls, Dialogs, ExtCtrls, Forms, Graphics,
   Messages, Spin, StdCtrls, SysUtils, UnNetBehaviour, UnNetRep, Variants, Windows;
...
//**** Take it out of here
  ...
  if RadioGroup1.ItemIndex = 0 then
     ...
     for i := 0 to FFInputsCount - 1 do
       XNewValues[i] := StrToFloat(FInMinEdits[i].Text);
     FNetwork.ChangeInputMin(XNewValues);
     for i := 0 to FFInputsCount - 1 do
       XNewValues[i] := StrToFloat(FInMaxEdits[i].Text);
     FNetwork.ChangeInputMax(XNewValues);
     SetLength(XNewValues, FFOutputsCount);
     for i := 0 to FFOutputsCount - 1 do
       XNewValues[i] := StrToFloat(FOutMinEdits[i].Text);
     FNetwork.ChangeOutputMin(XNewValues);
     for i := 0 to FFOutputsCount - 1 do
       XNewValues[i] := StrToFloat(FOutMaxEdits[i].Text);
...
procedure TFmCross.LoadIoInterface;
  ...
  for i := 0 to FNetRep.GetInputCount - 1 do
    ...
    FInMinEdits[i] := TEdit.Create(self);
    with FInMinEdits[i] do
      ...
      width := 60;
      height := 21;
      text := FloatToStr(FNetRep.Input[i].InMin);
      end;
    FInMaxEdits[i] := TEdit.Create(self);
    with FInMaxEdits[i] do
      ...
      width := 60;
      height := 21;
      text := FloatToStr(FNetRep.Input[i].InMax);
      end;
    end;
  ...
  for i := 0 to FNetRep.GetOutputCount - 1 do
    ...
    FOutMinEdits[i] := TEdit.Create(self);
    with FOutMinEdits[i] do
      ...
      width := 60;
      height := 21;
      text := FloatToStr(FNetRep.Output[i].OutMin);
      end;
    FOutMaxEdits[i] := TEdit.Create(self);
    with FOutMaxEdits[i] do
      ...
      width := 60;
      height := 21;
      text := FloatToStr(FNetRep.Output[i].OutMax);
      end;
    end;
  end;
