...
type TRafaANN = class
  ...
  public
    ...
    function NewChecking(ChkFile, RelatFile : TFileName; SelectiveMSE, diffOrder : boolean; CorrInt : double = 0.5) : double;
...
function  TRafaANN.NewChecking(ChkFile, RelatFile : TFileName; SelectiveMSE, diffOrder : boolean; CorrInt : double = 0.5) : double;
  var
  ...
  for xLine := 0 to XDataFile.Count - 1 do
    ...
    if XTmpData.Count <= FNoInputs then //Temporarily considered the reset condition
       ...
       if (not SelectiveMSE) or (c1 = '+') then
          ...
          if XAllCorrect then XCorrect[FNoOutputs] := XCorrect[FNoOutputs];
          tmpMse := tmpMse + (tmp / FNoOutPuts);
          FCasesCount := FCasesCount + 1;
          end;
       end;
