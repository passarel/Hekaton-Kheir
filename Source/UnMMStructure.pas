...
uses Classes, SysUtils, SimpleGraph, UnNetBehaviour, UnRafaAux2007, UnLogSing, UnLabelInformation, ComCtrls;
...
TRafaMooreMachine = class
  private
    //Boolean variable indicating the use of labels
    FUsesLabels : boolean;                                    
...
//  function  GetOutputSize : integer;
  ...
  public
    ...
    procedure   LoadFromVariables(InputVar, StateVar, OutputVar : TStrings; nHid : integer = -1);
...
//  property  OutputSize : integer read GetOutputSize write SetOutputSize;
    ...
    property  RandomizeOrder : boolean read FRandomizeOrder write FRandomizeOrder;
    property  UsesLabels : boolean read FUsesLabels write FUsesLabels;
...
//****    function SaveToFile(fileName : string ): boolean;
    ...
    procedure UpdateSequencesTreeList(treeList : TTreeView);
...
procedure TRafaMooreMachine.AddDataToHistory(arInput, arOutput : array of double);
  ...
  for i := 0 to FOutputVariables.Count - 1 do
    FHistory[XIndex, FInputVariables.Count + (2 * FStateVariables.Count) + i] := arOutput[FStateVariables.Count + i];
...
procedure TRafaMooreMachine.AddDataToHistoryWithDes(arInput, arOutput, arDesOutput : array of double);
  ...
  if FKeepSupervisionHistory then
     for i := 0 to FStateVariables.count + FOutputVariables.Count - 1 do
...
function TRafaMooreMachine.AddTransition(tr : TRafaTransition) : integer;
  ...
  if FUsesLabels then UpdateGraphWeights;
...
constructor TRafaMooreMachine.Create(graph : TSimpleGraph);
  ...
  FOutputVariables := TStringList.Create;
  FUsesLabels := true;  
...
procedure TRafaMooreMachine.Execute;
  var
    ...
    XRndArray : array of integer;
    XRndStrArray : array of string;

  begin

  XCurrentState := FInitialState;
  SetLength(inAr, FInputVariables.Count + FStateVariables.Count);
  SetLength(outAr1, FStateVariables.Count + FOutputVariables.Count);
  SetLength(outAr2, FStateVariables.Count + FOutputVariables.Count);
  XExecCount := 0;
  ...
  while FCanExecute and (FEpochCount < FLastEpoch) do
    ...
    if FRandomizeOrder then
      begin
      SetLength(XRndArray, FDataList.Count);
      SetLength(XRndStrArray, FDataList.Count);
      XRandom.RandomIntegerArray(XRndArray, 0, FDataList.Count);
      for i := 0 to FDataList.Count - 1 do
        XRndStrArray[i] := FDataList.Strings[XRndArray[i]];
      for i := 0 to FDataList.Count - 1 do
        FDataList.Strings[i] := XRndStrArray[i];
      end;
    ...
    while FExecutionCount < FEpochSize do
      ...
      FNetwork.GetOutputArray(outAr1, FStateVariables.Count + FOutputVariables.Count);
...
procedure   TRafaMooreMachine.LoadFromVariables(InputVar, StateVar, OutputVar : TStrings; nHid : integer = -1);
  var
    i : integer;
    XNet : TNetworkRep;
    XNeu : TNeuronRep;
  ...
  for i := 0 to OutputVar.Count - 1 do
    FOutputVariables.Add(OutputVar.Strings[i]);
  ...
  XNeu.BiasWeight := 0;
  XNeu.RandomBias := 1;
  XNet.LoadFeedForward(3, [InputVar.Count + StateVar.Count, nHid, StateVar.Count + OutputVar.Count], XNeu);
...
procedure TRafaMooreMachine.LoadNetwork(FileName : TFileName; extraNeurons : integer = 0);
  ...
  if ((XNet.GetInputCount - (XNet.GetOutputCount - FOutputVariables.Count)) <> FInputVariables.Count) or
     (XNet.GetOutputCount <> (FStateVariables.Count + FOutputVariables.Count)) then
     begin
     FInputVariables.Clear;
     FStateVariables.Clear;
     if (XNet.GetInputCount > 20) and (FUsesLabels) then
        raise Exception.Create(' Sorry, but our full model is limited to a maximum of 20 variables (states + input). \n If you want to use more variables, disable the use of labels');
...
procedure TRafaMooreMachine.SetHistorySize(value : integer);
  ...
  if value >= 0 then
     ...
     SetLength(FHistory, FHistorySize, (2 * FStateVariables.Count) + FInputVariables.Count + FOutputVariables.Count);
     if FKeepSupervisionHistory then
        SetLength(FSupervisionHistory, FHistorySize, FStateVariables.Count + FOutputVariables.Count);
     end;
  end;
...
procedure TRafaMooreMachine.UpdateSequencesTreeList(treeList : TTreeView);
  var
