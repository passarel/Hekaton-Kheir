...
uses
  ...
  Dialogs, Menus, ComCtrls, SimpleGraph, UnMMStructure, StdCtrls,
  JvExStdCtrls, JvEdit, JvValidateEdit, ExtCtrls, UnNetRep, Spin,
  JvComCtrls, JvExComCtrls, TeeProcs, TeEngine, Chart, series, Gauges;
...
procedure TFmMMMain.UpdateSelectedStates(arr : array of integer);
  ...
  JvValidateEdit1.Value := XX;
  JvValidateEdit2.Value := XY;
...
procedure TFmMMMain.JvValidateEdit3Change(Sender: TObject);
  ...
  for i := 0 to ListBox6.Count - 1 do
    if ListBox6.Selected[i] then
       FStructure.GetTransition(i).Goal := JvValidateEdit1.Value;
  end;
...
procedure TFmMMMain.Button18Click(Sender: TObject);
  ...
  if RadioGroup2.ItemIndex  > 0 then
     ...
     JvValidateEdit6.Value := FStructure.GetDataList.Count;
     end
  else
     FStructure.SetEpochSize(JvValidateEdit6.Value);
  FStructure.SetLastEpoch(JvValidateEdit7.value);
...
procedure TFmMMMain.Button15Click(Sender: TObject);
  begin
  FNetwork.SetNEta(JvValidateEdit4.value);
  FNetwork.SetNMomentum(JvValidateEdit5.Value);
  end;
...
procedure TFmMMMain.JvValidateEdit6Change(Sender: TObject);
  begin
  if JvValidateEdit6.Value > 1 then
     SpinEdit1.MaxValue := JvValidateEdit6.Value;
  end;
...
procedure TFmMMMain.JvTreeView1Click(Sender: TObject);
  var
    ...
    i1, i2, i3 : integer;
  begin
  XTmpNode := JvTreeView1.Items[JvTreeView1.ItemIndex];
...
//                  FStructure.Free;
            ...
            if XTmpStrState <> nil then
               ...
               FStructure.LoadFromVariables(XTmpStrInput, XTmpStrState);
               UpdateGraphStatesList;
               UpdateFullStateList;
               UpdateGraphInputsList;
               UpdateFullInputList;
            ...
            if XTmpStr2.Count > 1 then
               ...
               JvValidateEdit4.value := StrToFloat(XTmpStr2.Strings[1]);
               end;
            i := i + 1;
            end
            ...
            if XTmpStr2.Count > 1 then
               ...
               JvValidateEdit5.Value := StrToFloat(XTmpStr2.Strings[1]);
               end;
            i := i + 1;
            end
            ...
            if XTmpStr2.Count > 1 then
               begin
               JvValidateEdit7.Value := strtoint(XTmpStr2.Strings[1]);
               end;
            if XTmpStr2.Count > 2 then
               begin
            ...
            if RadioGroup2.ItemIndex  > 0 then
               ...
               if XTmpStr2.Count > 4 then
                  begin
                  JvValidateEdit6.Value := strtoint(XTmpStr2.Strings[4]);
...
procedure TFmMMMain.JvValidateEdit1Exit(Sender: TObject);
  var i : integer;
  begin
  if JvValidateEdit1.Value >= 0 then
     for i := 0 to ListBox2.Count - 1 do
       if ListBox2.Selected[i] then
          FStructure.XPosition[i] := JvValidateEdit1.Value;
  end;
...
procedure TFmMMMain.JvValidateEdit2Exit(Sender: TObject);
  var i : integer;
  begin
  if JvValidateEdit2.Value >= 0 then
     for i := 0 to ListBox2.Count - 1 do
       if ListBox2.Selected[i] then
          FStructure.YPosition[i] := JvValidateEdit2.Value;
  end;
